// Forager.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_FORAGER, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_FORAGER));
           ...
           if (bCheckFPS)
            {
             ...
             else if (dwTime + 10 < (DWORD)GetTickCount64())
            {
                MainGame.Update();
                MainGame.Late_Update();
                MainGame.Render();
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_FORAGER));
