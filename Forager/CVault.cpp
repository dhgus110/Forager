...
CVault::CVault() :m_dwIncomeTime((DWORD)GetTickCount64()), m_iIncome(0), m_bIncome(false), m_dwCoinRenderLifeTime(0), m_fRaise(0.f)
{
}
...
void CVault::Late_Update()
{
    Income();
    Coin_LifeCycle();
}

void CVault::Render(HDC hDC)
{
...
void CVault::Render_Coin(HDC hDC)
{
    if (!m_bIncome) return;
