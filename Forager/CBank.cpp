...
#include "CSoundMgr.h"
CBank::CBank() :m_dwIncomeTime((DWORD)GetTickCount64()), m_iIncome(0), m_bIncome(false), m_dwCoinRenderLifeTime(0), m_fRaise(0.f)
{
}
...
void CBank::Late_Update()
{
    Income();
    Coin_LifeCycle();
}

void CBank::Render(HDC hDC)
{
...
void CBank::Render_Coin(HDC hDC)
{
    if (!m_bIncome) return;
